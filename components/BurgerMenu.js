...
export default function BurgerMenu({className = '', active, setBurgerActive, ...rest}) {
                    ...
                    {locales.map((localeItem, key) => {
                        return (
                            <Link href={asPath} locale={localeItem} key={`NavLang: ${localeItem}`}>
                                <a className={`flex items-center gap-2 text-white hover:text-white/60 transition-colors duration-200 ${!active ? 'opacity-0 -translate-y-1/2' : ''}`}
                                   onClick={() => {
                                       setLocaleCookies(localeItem);
                                       handleClickLink();
                                   }}
                                   style={{
                                       transition: active ? `opacity 0.3s ease ${(allLinks.length + 2 + key) * 0.15}s, transform 0.3s ease ${(allLinks.length + 2 + key) * 0.15}s, color 0.3s ease, background 0.3s ease` : 'opacity 0.3s ease, transform 0s ease 0.3s',
                                   }}
                                >
